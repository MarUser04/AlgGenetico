...
class Selection:
    ...
    def cal_selection(self):
        ...
        for i in range(4):
            population_random = random.sample(remaining, min(len(remaining), 25))
            self.population['population_{}'.format(i)] = population_random
    ...
    def worst_fitness(self):
        minimums = []
        for i in range(4):
            minimums.append(self.fitness.fitness.index(min(self.population['population_{}'.format(i)])))
        min1 = min(minimums)
